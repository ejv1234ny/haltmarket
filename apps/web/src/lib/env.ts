export const env = {
  NEXT_PUBLIC_SUPABASE_URL: process.env.NEXT_PUBLIC_SUPABASE_URL ?? '',
  NEXT_PUBLIC_SUPABASE_ANON_KEY: process.env.NEXT_PUBLIC_SUPABASE_ANON_KEY ?? '',
  NEXT_PUBLIC_SITE_URL: process.env.NEXT_PUBLIC_SITE_URL ?? 'http://127.0.0.1:3000',
  // Server-only. Used exclusively for leaderboard-style aggregations over
  // every user, which RLS can't satisfy with the anon client. Never
  // exposed to the browser — only read from server components.
  SUPABASE_SERVICE_ROLE_KEY: process.env.SUPABASE_SERVICE_ROLE_KEY ?? '',
};

// When Supabase credentials are absent (local dev without a project yet, or
// CI), the UI runs against fixture data. The flag lets components branch
// without swallowing real errors once credentials land.
export const supabaseConfigured =
  env.NEXT_PUBLIC_SUPABASE_URL.length > 0 && env.NEXT_PUBLIC_SUPABASE_ANON_KEY.length > 0;

export const serviceRoleConfigured =
  supabaseConfigured && env.SUPABASE_SERVICE_ROLE_KEY.length > 0;
